...
internal class RootWriter(RootMetadata metadata) : BaseWriter<RootMetadata, BaseWriter>(metadata, null!)
{
    ...
    {
        Builder.Append(MarkdownHelper.Header(1)).Append(' ').AppendLine("此文档包含如下程序集");
        ...
        Metadata.Assemblies.ForEach(x =>
        {
            ...
            Builder.Append(MarkdownHelper.UnOrderedList(1)).Append(' ').AppendLine(MarkdownHelper.HyperLink($"{x.AssemblyInfo.Name}.dll", MarkdownPath.GetUrlRelativePath(assemblyWriter.MarkdownPath)));
        });
    }
}
